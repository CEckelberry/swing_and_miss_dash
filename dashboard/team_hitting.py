...
def fetch_data(season):
    if season >= current_year:
        ...
        query = f"""
        SELECT 
            Season, `Team`, AVG, OBP, SLG, `wRC_` AS `wRC+`, wOBA, OPS, BABIP, WAR, 
            `K_` AS `K%`, `BB_` AS `BB%`, HR, Def, SB, CS, BsR, `3B` AS `Triples`, 
            `2B` AS `Doubles`, RBI, `H` AS Hits, Age
        FROM 
            `raw_data`.`team_batting_stats`
        WHERE 
            `Season` = {season}
            AND `upload_date` = (
