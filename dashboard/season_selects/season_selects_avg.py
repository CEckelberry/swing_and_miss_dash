...
# Create two columns
    ...
    with col1:
        selected_season_start = st.selectbox("League Average Season Start", options=seasons, key="season_start")
    with col2:
        print()
    # Use the second column for the second select box
    with col3:
        print()
        selected_season_end = st.selectbox("League Average Season End", options=seasons, key="season_end")
    with col4:
        print()
